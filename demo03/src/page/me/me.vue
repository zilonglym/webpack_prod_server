<template>
  ...
  <div class="my-header clearfix">
    ...
    <!--</div>-->
      <div class="picLayout">
          <img src="../../images/mrtx.png" alt="" v-if="!userInfo.headimgurl">
          <img :src="userInfo.headimgurl" v-else alt="">
      </div>
      <div class="ac white pb30" v-if="userInfo.headimgurl">
          {{userInfo.nickname}}
      </div>
      <div class="ac white pb30" v-else>
          {{userInfo.mobile}}
      </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods:{
        initData(){
            var _self = this;
            if(_self.isweixin() && JSON.parse(getStore("wxUserInfor"))){
                _self.userInfo = JSON.parse(getStore("wxUserInfor"));
            }else{
                getUserInfo({
                    success:(data)=>{
                        _self.userInfo = data;
            ...
            var fetchData = getMyNum({
                success:(data)=>{
                    _self.meshowStatus=true;
                    _self.myNum = data;
                }
            });
            if(fetchData == 'notLogin' ){
                _self.meshowStatus=true;
            }
        },
        isweixin(){
